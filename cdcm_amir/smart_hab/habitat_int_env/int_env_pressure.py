...
def make_int_env_pressure_env_0(
    dome_specs,
    en_used_pres,
    en_needed_pres,
    structure_sec_1,
    ...
    with System(
        name="int_env_pressure", description="The int_env_pressure environment"
    ) as int_env_pressure:
        int_env_pres = State(
            name="int_env_pres",
            value=1.0,
        ...
        def f_interior_env_pres(
            ...
            if en_needed_pres == 0:
                int_env_pres_new = (lower_pressure_setpo + upper_pressure_setpo) / 2
            else:
                int_env_pres_new = max(
                    0.0,
                    min(
                        1.0,
                        max(en_used_pres, 1 * 10 ** (-10))
                        / max(en_needed_pres, 1 * 10 ** (-4)),
                    )
                    ...
                    * (
                        ((1.0 - latent_struct_int_env) * air_leak_coeficent + 0)
                        * out_of_str_pres
                    ),
                )
