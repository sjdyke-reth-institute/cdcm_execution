...
__all__ = ["make_struct_temp_env_0"]


def make_struct_temp_env_0(
    dome_specs, irradiance, surface_temperature, struct_health, int_env_temp
):
    with System(
        name="struct_temp", description="The struct_temp environment"
    ) as struct_temp:
        ...
        def f_struct_temp(
            ...
            external_temp=surface_temperature,
            ...
            ext_str_temp_new = (
                (
                    ...
                    * (
                        0.001
                        + abs(
                            (int_str_temp - ext_str_temp)
                            * (
                                nominal_cond_coef * struct_latent
                                + damaged_cond_coef * (1.0 - struct_latent)
                            )
                            / dom_thickness
                        )
                    )
                    ...
                    * (
                        0.001
                        + abs(
                            (irradiation + np.power(external_temp, 4))
                            * surf_absorb_coef
            ...
            int_str_temp_new = (
                ...
                * (
                ...
                / dom_thickness
                + 1.0 * int_env_temp * int_conv_coef
            ) / (
                1.0
                * (
                    nominal_cond_coef * struct_latent
            ...
            return ext_str_temp_new, int_str_temp_new
