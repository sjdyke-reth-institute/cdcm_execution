...
def make_eclss_temperature_env_0(
    clock,
    dome_specs,
    eclss_pressure,
    ...
    with System(
        name="eclss_temperature", description="The eclss_temperature environment"
    ) as eclss_temperature:
        ...
        @make_function(en_needed_heat, en_used_heat)
        def f_eclss_heat(
            ...
            en_needed_heat_new = abs(
                (
                    ((lower_temp_setpo + upper_temp_setpo) / 2 - int_env_temp)
                    * air_heat_capac
                    - dt * (int_str_temp - int_env_temp) * int_conv_coef
                )
                / efficiency_of_TM
            )
            en_used_heat_new = max(
                0.0, min(available_en - en_used_pres, en_needed_heat)
            )
            return en_needed_heat_new, en_used_heat_new
