...
def make_HM_physical_env_0(clock,
                        dome_specs,
                        moon,
                        energy,
                        eclss,
    ...
    with System(name="HM_physical", description="The HM_physical environment") as HM_physical:
        ...
        agents_command = Variable(name="agents_command",
                                 value=False,
                                 units="",
        ...
        def f_physical(clock=clock,
            ...
            while retask:
                for fault in health_managment_detection:
                    list_time_to_failures.append(health_managment_detection.time_to_failure)
                idx_list = np.argsort(list_time_to_failures)
                retask = False
                for idx in idx_list:
                    if not(retask):
                        for agent in agents:
                            if agent.available or health_managment_detection[idx].time_to_failure < agent.task_at_hand.time_to_failure:
