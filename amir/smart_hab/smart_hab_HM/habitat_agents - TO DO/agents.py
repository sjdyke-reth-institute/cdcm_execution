...
def make_eclss(clock,
    ...
    with System(name="eclss", description="The eclss system") as eclss:
        ...
        if interior_env_pressure is None:
        ...
        if HM_temperature_lower_setpoint is None:
            HM_temperature_lower_setpoint = Variable(name="place_holder_HM_lower_temparature_setpoint", units="", description="The lower temperature set point from HM to ECLSS")
        if HM_temperature_upper_setpoint is None:
            HM_temperature_upper_setpoint = Variable(name="place_holder_HM_upper_temparature_setpoint", units="", description="The upper temperature set point from HM to ECLSS")
        if HM_pressure_lower_setpoint is None:
            HM_pressure_lower_setpoint = Variable(name="place_holder_HM_lower_pressure_setpoint", units="", description="The lower pressure set point from HM to ECLSS")
        if HM_pressure_upper_setpoint is None:
            HM_pressure_upper_setpoint = Variable(name="place_holder_HM_upper_pressure_setpoint", units="", description="The upper pressure set point from HM to ECLSS")



        eclss_pressure = make_eclss_pressure_env(dome_specs, energy_available_energy, struct_health, interior_env_pressure, HM_pressure_lower_setpoint, HM_pressure_upper_setpoint)

        eclss_temperature = make_eclss_temperature_env(clock, dome_specs, eclss_pressure, energy_available_energy, struct_inside_temperature, interior_env_temperature,
                                                        HM_temperature_lower_setpoint, HM_temperature_upper_setpoint)

        eclss_energy_consumption = make_eclss_energy_consumption_env(energy_available_energy, eclss_pressure, eclss_temperature)
