...
def make_structure_health_env_0(moon, agent_repair_struct):
    with System(
        name="structure_health", description="The structure_health environment"
    ) as structure_health:
        ...
        def f_struct_health(
            ...
            struct_1_T = min(
                max(
                    structure_sec_1 - meteorite_impact_sec_1 + fix_structure_sec_1,
                    0.001,
                ),
                1.0,
            )
            struct_2_T = min(
                max(
                    structure_sec_2 - meteorite_impact_sec_2 + fix_structure_sec_2,
                    0.001,
                ),
                1.0,
            )
            struct_3_T = min(
                max(
                    structure_sec_3 - meteorite_impact_sec_3 + fix_structure_sec_3,
                    0.001,
                ),
                1.0,
            )
            struct_4_T = min(
                max(
                    structure_sec_4 - meteorite_impact_sec_4 + fix_structure_sec_4,
                    0.001,
                ),
                1.0,
            )
            struct_5_T = min(
                max(
                    structure_sec_5 - meteorite_impact_sec_5 + fix_structure_sec_5,
                    0.001,
                ),
                1.0,
            )
