...
__all__ = [
    "make_battery_health_degaradation_env_linear",
    "make_battery_health_degaradation_env_exponential_decay",
    "make_battery_health_degaradation_env_matrix_probability",
]


def make_battery_health_degaradation_env_linear(clock, battery_specs):
    with System(
        name="battery_degerade", description="The battery_degerade environment"
    ) as battery_degerade:
        ...
        def f_battery_degeradation_state(
            battery_degeradation_state=battery_degeradation_state,
            dt=clock.dt,
            battery_nominal_life=battery_nominal_life,
        ):
            """Transition function for battery_degeradation_state"""

            battery_degeradation_state_new = max(
                min(1.0, battery_degeradation_state - (dt / battery_nominal_life), 0.0)
            )
...
def make_battery_health_degaradation_env_exponential_decay(clock, battery_specs):
    with System(
        name="battery_degerade", description="The battery_degerade environment"
    ) as battery_degerade:
...
def make_battery_health_degaradation_env_matrix_probability(clock, battery_specs):
    with System(
        name="battery_degerade", description="The battery_degerade environment"
    ) as battery_degerade:
        ...
        probability_matrix = Parameter(
            ...
            value=np.array(
                [
                    [0.999726, 0.000243, 0.000031, 0.000000, 0.000000],
