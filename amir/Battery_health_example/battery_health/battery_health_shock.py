...
__all__ = ["make_battery_health_shock_current_env_without_cooling",
...
def make_battery_health_shock_current_env_without_cooling(battery_specs,
                                                          high_current,
                                                          battery_health_degaradation_state):
    with System(name="battery_shocked",
                description="The battery_shocked environment") as battery_shocked:
        ...
        def f_battery_shock_current_state(battery_shock_current_state=battery_shock_current_state,
            ...
            if high_current >= battery_ultimate_max_current:
                ...
                battery_shock_current_state_new = max(0.0, battery_shock_current_state - battery_short_time_max_current_damage)
            elif high_current >= battery_nominal_max_current:
                battery_shock_current_state_new = max(0.0, battery_shock_current_state - battery_nominal_max_current_damage)
            else:
...
def make_battery_health_shock_current_env_with_cooling(battery_specs,
                                                       high_current,
                                                       battery_health_degaradation_state):
    with System(name="battery_shocked",
                description="The battery_shocked environment") as battery_shocked:
        ...
        def f_battery_shock_current_state(battery_shock_current_state=battery_shock_current_state,
            ...
            if high_current >= battery_ultimate_max_current * current_allowance_gain:
                battery_shock_current_state_new = 0.0
            elif high_current >= battery_short_time_max_current * current_allowance_gain:
                battery_shock_current_state_new = max(0.0, battery_shock_current_state - battery_short_time_max_current_damage)
...
def make_battery_health_shock_current_env_without_cooling_degraded_life(battery_specs,
                                                                        high_current,
                                                                        battery_health_degaradation_state):
    with System(name="battery_shocked",
                description="The battery_shocked environment") as battery_shocked:
        ...
        def f_battery_shock_current_state(battery_shock_current_state=battery_shock_current_state,
            ...
            if high_current >= battery_ultimate_max_current:
                ...
                battery_shock_current_state_new = max(0.0, battery_shock_current_state - battery_short_time_max_current_damage * battery_degeradation_decay / (0.001 + battery_health_degaradation_state))
            elif high_current >= battery_nominal_max_current:
                battery_shock_current_state_new = max(0.0, battery_shock_current_state - battery_nominal_max_current_damage * battery_degeradation_decay / (0.001 + battery_health_degaradation_state))
            else:
                battery_shock_current_state_new = battery_shock_current_state
            return battery_shock_current_state_new
...
def make_battery_health_shock_current_env_with_cooling_degraded_life(battery_specs,
                                                                     high_current,
                                                                     battery_health_degaradation_state):
    with System(name="battery_shocked",
                description="The battery_shocked environment") as battery_shocked:
