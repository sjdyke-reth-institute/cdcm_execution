...
def make_int_env_temperature_env_0(
    en_used_heat,
    en_needed_heat,
    ...
    with System(
        name="int_env_temperature", description="The int_env_temperature environment"
    ) as int_env_temperature:
        int_env_temp = make_node(
            "S:int_env_temp",
            value=298.0,
            units="K",
        ...
        def f_interior_env_heat(
            en_used_heat=en_used_heat,
            ...
            # print(en_used_heat, en_needed_heat, int_str_temp, lower_temp_setpo, upper_temp_setpo)
            int_env_temp_new = max(
                100.0,
                min(
                    400.0,
                    1.0
                    * (
                        min(
                            1.0,
                            max(en_used_heat, 1 * 10 ** (-4))
                            / max(en_needed_heat, 1 * 10 ** (-4)),
                        )
                    )
                    * ((lower_temp_setpo + upper_temp_setpo) / 2)
                    + 1.0
                    * (
                        1
                        - min(
